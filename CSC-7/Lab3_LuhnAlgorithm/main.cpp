...
void flipDig(CrdCard,char[]);  //Randomly changes two digits in card number
void trnsDig(CrdCard,char[]);  //Randomly transposes two digits in cc number
        ...
        switch(choice){
            ...
            case 1:
                ...
                for(int i=0;i<repeat;i++){
                    ...
                    genCC(visa,visaCrd);
                    //Transpose random digits
                    trnsDig(visa,visaCrd);
                    //Flip random digits
...
void flipDig(CrdCard cc,char ccnum[]){
    int rndDig;  //Selects place of digit being changed
    int changed; //Holds place of digit being changed
    int count=0; //Counts the number of digits that have been changed
    do{
        //Choose random digit to change
        rndDig=rand()%cc.length;
        //Check if a new digit has been selected
        if(changed!=rndDig){
            //Hold place in credit card number
            changed=rndDig;
            //Change digit to new random digit
            ccnum[rndDig]=rand()%10+48;
            //Increment number of digits changed
            count++;
        }
        else{    
            ccnum[rndDig]=ccnum[rndDig];
        }      
    }while(count<2);    
}

void trnsDig(CrdCard cc,char ccnum[]){
    int rndDig;  //Selects place of digit being changed
    int changed; //Holds place of digit being changed
    int count=0; //Counts the number of digits that have been changed
    do{
        ...
        if(changed!=rndDig){
            //Hold place in credit card number
            changed=rndDig;
            //Change digit to either one above or one below
            rand()%2==0?ccnum[rndDig]=ccnum[rndDig]+1
                       :ccnum[rndDig]=ccnum[rndDig]-1;
            //Increment number of digits changed
            count++;
        }
        else{    
            //Defaults value to original value
            ccnum[rndDig]=ccnum[rndDig];
        }      
    }while(count<2); //Loops until two digits have been changed    
}